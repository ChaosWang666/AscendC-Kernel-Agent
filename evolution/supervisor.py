...
class EvolutionState:
    ...
    def format_lineage(self) -> str:
        ...
        lines = ["版本 | 评分 (TFLOPS) | Git Commit | 描述",
                  "-----|---------------|------------|------"]
        for entry in self.lineage:
            ...
            lines.append(f"v{v}   | {s:<13.2f} | {c}    | {d}")
        return "\n".join(lines)


# ========== Agent 会话 ==========

def read_current_kernel(state: EvolutionState, project_root: str) -> str:
    """读取当前内核源码"""
    kernel_path = os.path.join(
        project_root, "workspace", "ops",
        state.operator_name, f"{state.operator_name}.asc"
    )
...
def build_agent_prompt(state: EvolutionState, project_root: str,
                       config: dict, directive: str = None) -> str:
    """构建 Agent 会话的 prompt"""
    ...
    if state.current_version < 0:
        template_path = os.path.join(
            project_root, "agents/kernel-evolution-agent/prompts/seed-generation.md"
        )
    else:
        template_path = os.path.join(
            project_root, "agents/kernel-evolution-agent/prompts/optimize-step.md"
        )
    ...
    # 变量替换
    current_kernel = read_current_kernel(state, project_root)
...
def launch_agent_session(prompt: str, config: dict, project_root: str,
    ...
    try:
        ...
        return {"committed": False, "description": "会话超时", "log": log_path}

    except FileNotFoundError:
        print("错误: claude 命令未找到，请确保 Claude Code CLI 已安装")
        print("降级模式: 请手动执行 Agent 步骤")
        with open(log_path, "a") as f:
            f.write("## 错误\n\nclaude CLI 未找到\n")
        return {"committed": False, "description": "claude CLI 未找到", "log": log_path}
    ...
    committed = (
        score.get("correctness_total", 0.0) >= 1.0
        and score.get("performance_total_tflops", 0.0) > 0
    )
    ...
    return {
        ...
        "score": score,
        "description": commit_msg or score.get("version", ""),
        "log": log_path,
    }
...
def generate_redirect_directive(state: EvolutionState, project_root: str,
    ...
    调用 Claude 分析谱系和 profiling 数据，提出新的探索方向。
    """
    print("\n⚠️  检测到停滞，生成重定向指令...")
...
def should_stop(state: EvolutionState, config: dict) -> tuple:
    ...
    # 目标性能达成
    target = config.get("target_tflops")
    if target and state.best_score >= target:
        return True, f"目标性能已达成 ({state.best_score} >= {target} TFLOPS)"

    # 连续重定向失败
    max_redirects = config.get("max_consecutive_redirects", 3)
    if state.consecutive_redirects >= max_redirects:
        return True, f"连续 {max_redirects} 次重定向无效"
...
def main_loop(config: dict, project_root: str):
    ...
    print(f"最佳评分: {state.best_score} TFLOPS")
    print(f"谱系长度: {len(state.lineage)}")
    print("========================================\n")

    step_num = state.total_attempts
    ...
    while True:
        ...
        step_num += 1
        state.total_attempts = step_num
        directive = None

        # 停滞检测
        stall_threshold = config.get("stall_threshold", 5)
        max_failed = config.get("max_failed_attempts", 10)
        ...
        # 构建 prompt
        prompt = build_agent_prompt(state, project_root, config, directive)

        # 启动 Agent 会话
        result = launch_agent_session(prompt, config, project_root, step_num)

        # 处理结果
        if result["committed"]:
            state.current_version += 1
            new_score = result["score"].get("performance_total_tflops", 0.0)
            ...
            if new_score > state.best_score:
                print(f"\n🎉 新最佳! v{state.current_version}: {new_score} TFLOPS "
                      f"(+{new_score - state.best_score:.2f})")
                state.best_version = state.current_version
                state.best_score = new_score
                state.stall_counter = 0
                state.consecutive_redirects = 0
            else:
                print(f"\n📊 v{state.current_version}: {new_score} TFLOPS "
                      f"(未超越最佳 {state.best_score})")
                state.stall_counter += 1

            # Git tag
            try:
                tag = f"v{state.current_version}-score-{new_score}"
                subprocess.run(
                    ["git", "tag", tag],
