...
### α resolver(Item 4/P6 共享工具)

```python
def resolve_alpha(q_update_cfg, visit_k):
    """
    Item 4/P6 — α adaptive schedule:
    - alpha_mode="constant" 用固定 q_update_cfg.alpha(向后兼容)
    - alpha_mode="visit_count_adaptive":alpha_eff = max(floor, init / (1 + visit))
      visit=0 时 α=init(相当于首次观测作为 MC bootstrap);
      visit 大时收敛到 floor 保持对漂移敏感。
    """
    mode = q_update_cfg.get("alpha_mode", "constant")
    if mode == "constant":
        return q_update_cfg["alpha"]
    if mode == "visit_count_adaptive":
        init = q_update_cfg["alpha_init"]
        floor = q_update_cfg["alpha_floor"]
        return max(floor, init / (1 + visit_k))
    raise ValueError(f"unknown q_update.alpha_mode: {mode}")
```
...
def update_stage1(step, state, action, observation, reward, context_items):
    ...
    for item in context_items:
        ...
        # Item 4/P6 — α adaptive:随 visit_1 递减,加速早期收敛再稳定
        alpha = resolve_alpha(config.q_update, visit_k=entry["visit_1"])
...
def update_stage2(step, state, action, observation, reward_raw,
    ...
    # ⚠ Item 1/P6 口径约定:observation.latency_us 必须来自 multigate-verifier,
    #    即从 score_{t}.json.performance_primary 读(R9+R12 后该字段即 median_ms * 1000)。
    #    reward_raw 上游已计算为 tanh(log(b_t) - log(latency_us)),b_t 同 median 口径。
    #    新 bank 条目的 meta.latency_us / meta.latency_measure_type 由此传播为 "median"。
    # 1. PopArt 归一化
    ...
    q = load("evo/memory/q_values.json")
    q_clip = config.q_update.q_clip
    # Item 4/P6 — α adaptive:每个 item 独立用 visit_2 计算 alpha_eff(见循环内)
    ...
    for item in affected:
        ...
        if is_seed_bypass:
            ...
            # Item 4/P6 — α adaptive:用当前 entry.visit_2(已 +=1)计算 alpha_eff
            alpha = resolve_alpha(config.q_update, visit_k=entry["visit_2"])
